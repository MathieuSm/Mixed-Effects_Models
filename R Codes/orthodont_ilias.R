...
ortho.lme.3 <- lme(distance ~ Sex * I(age-11), random = ~ I(age-11) | Subject, 
  weights = varIdent(form = ~ 1 | Sex), data=orthodont.df)

summary(ortho.lme.3)

# random effects could still be assumed to be uncorrelated:
ortho.lme.3a <- lme(distance ~ Sex * I(age-11), random = list(Subject = pdDiag(~ I(age-11))), 
  weights = varIdent(form = ~ 1 | Sex), data=orthodont.df)
anova(ortho.lme.3, ortho.lme.3a)

# scatter plots of residuals vs. fitted values, by sex
plot(ortho.lme.3, resid(., type='pearson') ~ fitted(.) | Sex, grid=FALSE, abline=0, lty='dotted')
# compare to:
plot(ortho.lme.2, resid(., type='pearson') ~ fitted(.) | Sex, grid=FALSE, abline=0, lty='dotted')

if (FALSE) {
postscript(file = "U:/vock/lehre/mixed_models/tex/ortho.lme.3.res.by.sex.eps", onefile=TRUE,
  paper="special", width=7, height=5, family="Times", horizontal=FALSE)
par(oma=c(0,0,0,0), mar=c(3,3,0.2,0.2), mgp=c(1.8, 0.7, 0), cex=1)
plot(ortho.lme.3, resid(., type='pearson') ~ fitted(.) | Sex, grid=FALSE, abline=0, lty='dotted', scales=list(cex=1.2), 
  par.settings=list(axis.text=list(cex=1.2), par.xlab.text=list(cex=1.2), par.ylab.text=list(cex=1.2), add.text=list(cex=1.2)))
dev.off()
} # end if (FALSE)

qqnorm(ortho.lme.3, ~ resid(., type='pearson'))

anova(ortho.lme.2, ortho.lme.3)

# graphical comparison of the fitted lines of ortho.lme.2 and .3
plot(orthodont.df$age, orthodont.df$distance, xlab='age', ylab='distance', type='n', axes=FALSE)
axis(1, at=seq(8, 14, 2))
axis(2)
box()
for (s.index in 1:length(unique(orthodont.df$Subject))) {
  s <- unique(orthodont.df$Subject)[s.index]
  polygon(c(orthodont.df$age[orthodont.df$Subject==s], rev(orthodont.df$age[orthodont.df$Subject==s])),
          c(fitted(ortho.lme.2)[orthodont.df$Subject==s], rev(fitted(ortho.lme.3)[orthodont.df$Subject==s])),
          col=rainbow(27)[s.index], density=30, angle=4*s.index+30)
}
for (s.index in 1:length(unique(orthodont.df$Subject))) {
  s <- unique(orthodont.df$Subject)[s.index]
  lines(orthodont.df$age[orthodont.df$Subject==s], fitted(ortho.lme.3)[orthodont.df$Subject==s], col=rainbow(27)[s.index], lwd=2)
}
legend('bottomright', lty=1, lwd=c(1, 2), c('ortho.lme.2', 'ortho.lme.3'))


# further variance functions

ortho.lme.4 <- lme(distance ~ Sex * I(age-11), random = ~ I(age-11) | Subject, 
  weights = varIdent(form = ~ 1 | Sex * age), data=orthodont.df)
summary(ortho.lme.4)
anova(ortho.lme.3, ortho.lme.4)

ortho.lme.5 <- lme(distance ~ Sex * I(age-11), random = ~ I(age-11) | Subject, 
  weights = varComb(varIdent(form = ~ 1 | Sex), varIdent(form = ~ 1 | age)), 
  data=orthodont.df)
summary(ortho.lme.5)
anova(ortho.lme.3, ortho.lme.5)

# can't specify the omission of the interaction as follows:
ortho.lme.5a <- lme(distance ~ Sex * I(age-11), random = ~ I(age-11) | Subject, 
  weights = varIdent(form = ~ 1 | Sex + age), data=orthodont.df)  # will not work

ortho.lme.6 <- lme(distance ~ Sex * I(age-11), random = ~ I(age-11) | Subject, 
  weights = varFixed(~ age), data=orthodont.df)
summary(ortho.lme.6)
anova(ortho.lme.2, ortho.lme.6)  # no LRT -- same df...

ortho.lme.6a <- lme(distance ~ Sex * I(age-11), random = ~ I(age-11) | Subject, 
  weights = varPower(form = ~ age, fixed=0.5), data=orthodont.df)
summary(ortho.lme.6a)

ortho.lme.7 <- lme(distance ~ Sex * I(age-11), random = ~ I(age-11) | Subject, 
  weights = varPower(form = ~ age), data=orthodont.df)
summary(ortho.lme.7)
anova(ortho.lme.2, ortho.lme.7)

ortho.lme.8 <- lme(distance ~ Sex * I(age-11), random = ~ I(age-11) | Subject, 
  weights = varPower(form = ~ age | Sex), data=orthodont.df)
summary(ortho.lme.8)
anova(ortho.lme.2, ortho.lme.8)

ortho.lme.9 <- lme(distance ~ Sex * I(age-11), random = ~ I(age-11) | Subject, 
  weights = varComb(varPower(form = ~ age | Sex), varIdent(form = ~ 1 | Sex)), 
  data=orthodont.df)
summary(ortho.lme.9)
anova(ortho.lme.2, ortho.lme.9)

ortho.lme.10 <- lme(distance ~ Sex * I(age-11), random = ~ I(age-11) | Subject, 
  weights = varPower(form = ~ fitted(.)), data=orthodont.df)
summary(ortho.lme.10)
anova(ortho.lme.2, ortho.lme.10)

ortho.lme.11 <- lme(distance ~ Sex * I(age-11), random = ~ I(age-11) | Subject, 
  weights = varExp(form = ~ age), data=orthodont.df)
summary(ortho.lme.11)
anova(ortho.lme.2, ortho.lme.11)

ortho.lme.12 <- lme(distance ~ Sex * I(age-11), random = ~ I(age-11) | Subject, 
  weights = varConstPower(form = ~ fitted(.)), data=orthodont.df)
summary(ortho.lme.12)
anova(ortho.lme.2, ortho.lme.12)


##################################
# correlated within-group errors #
##################################

ACF(ortho.lme.3)
plot(ACF(ortho.lme.3), alpha=0.05)

# MA(1)
ortho.lme.4 <- lme(distance ~ Sex * I(age-11), random = ~ I(age-11) | Subject, 
  weights = varIdent(form = ~ 1 | Sex), 
  correlation = corARMA(form = ~ 1 | Subject, p=0, q=1), 
  data=orthodont.df)
# or:
# ortho.lme.4 <- lme(distance ~ Sex * I(age-11), random = ~ I(age-11) | Subject, 
#   weights = varIdent(form = ~ 1 | Sex), 
#   correlation = corARMA(form = ~ age/2 | Subject, p=0, q=1), 
#   data=orthodont.df)
...
ortho.lme.6 <- lme(distance ~ Sex * I(age-11), random = ~ I(age-11) | Subject, 
  ...
  data=orthodont.df)
summary(ortho.lme.6)
anova(ortho.lme.3, ortho.lme.6)

# general -- does not converge with the default settings
lme(distance ~ Sex * I(age-11), random = ~ I(age-11) | Subject, 
  weights = varIdent(form = ~ 1 | Sex), 
  correlation = corSymm(form = ~ 1 | Subject), 
  data=orthodont.df)
# increase the number of iterations for nlminb:
ortho.lme.7 <- lme(distance ~ Sex * I(age-11), random = ~ I(age-11) | Subject, 
  weights = varIdent(form = ~ 1 | Sex), 
  correlation = corSymm(form = ~ 1 | Subject), 
  data=orthodont.df, control=list(msMaxIter=100))
anova(ortho.lme.3, ortho.lme.7)
...
ortho.gls.1 <- gls(distance ~ Sex * I(age-11), 
  correlation = corSymm(form = ~ 1 | Subject), 
  weights = varIdent(form = ~ 1 | age), data=orthodont.df)

ortho.gls.1

ortho.gls.2 <- gls(distance ~ Sex * I(age-11), 
  correlation = corCompSymm(form = ~ 1 | Subject), 
  weights = varIdent(form = ~ 1 | age), data=orthodont.df)
ortho.gls.3 <- gls(distance ~ Sex * I(age-11), 
  correlation = corCompSymm(form = ~ 1 | Subject), data=orthodont.df)
anova(ortho.gls.1, ortho.gls.2, ortho.gls.3)

plot(ortho.gls.3, resid(., type='n') ~ age | Sex)

ortho.gls.4 <- gls(distance ~ Sex * I(age-11), 
  correlation = corCompSymm(form = ~ 1 | Subject), 
  weights = varIdent(form = ~ 1 | Sex), data=orthodont.df)
anova(ortho.gls.3, ortho.gls.4)

plot(ortho.gls.4, resid(., type='n') ~ age | Sex)


# combinations of heteroscadasticity for sex with the general var/cov matrix
ortho.gls.5 <- gls(distance ~ Sex * I(age-11), 
  correlation = corSymm(form = ~ 1 | Subject), 
  weights = varComb(varIdent(form = ~ 1 | age), varIdent(form = ~ 1 | Sex)),
  data=orthodont.df)
ortho.gls.6 <- gls(distance ~ Sex * I(age-11), 
  correlation = corSymm(form = ~ 1 | Subject), 
  weights = varIdent(form = ~ 1 | age*Sex),
  data=orthodont.df)
anova(ortho.gls.4, ortho.gls.5, ortho.gls.6)


# comparison to the LME model
anova(ortho.lme.3, ortho.gls.4, test=FALSE)


# comparison to an LME model with a random intercept only

ortho.lme.3.without.random.slope <- lme(distance ~ Sex * I(age-11), 
  random = ~ 1 | Subject,  
  weights = varIdent(form = ~ 1 | Sex), data=orthodont.df)

anova(ortho.lme.3.without.random.slope, ortho.gls.4, test=FALSE)
