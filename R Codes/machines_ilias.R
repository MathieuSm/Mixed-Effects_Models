...
# obtain the two-level model by a single-level model and a compound symmetry
# correlation structure

machines.lme.2c <- lme(score ~ Machine, random = ~ 1 | Worker,
  correlation = corCompSymm(form = ~ 1 | Worker/Machine), data=machines.df)
summary(machines.lme.2c)
