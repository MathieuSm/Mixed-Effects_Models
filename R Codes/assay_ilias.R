...
# (b)
# models that are equivalent to assay.lme.1, but have only 11 instead 
# of 12 random effect components

assay.lme.1a <- lme(logDens ~ sample * dilut, 
  random = list(Block = pdBlocked(list(pdCompSymm(~ sample - 1), 
                                       pdIdent(~ dilut - 1)))),
  data=assay.df)

assay.lme.1b <- lme(logDens ~ sample * dilut, 
  random = list(Block = pdBlocked(list(pdIdent(~ sample - 1), 
                                       pdCompSymm(~ dilut - 1)))),
  data=assay.df)

anova(assay.lme.1, assay.lme.1a, assay.lme.1b)

signif(getVarCov(assay.lme.1), 3)
signif(getVarCov(assay.lme.1a), 3)
signif(getVarCov(assay.lme.1b), 3)



# (c)
# same as assay.lme.1, but with an arbitrary diagonal Psi

assay.lme.6 <- lme(logDens ~ sample * dilut, 
  random = list(Block = pdBlocked(list(pdIdent(~ 1), pdDiag(~ sample - 1), 
                                       pdDiag(~ dilut - 1)))),
  data=assay.df)

assay.lme.6
# StdDev of certain random effects is estimated as almost 0

ranef(assay.lme.6)
# some predicted random effects are numerically 0

anova(assay.lme.1, assay.lme.6)
# doesn't fit significantly better than assay.lme.1
